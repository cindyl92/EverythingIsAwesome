...
public class BuildingHeight : MonoBehaviour {
	...
	void Start () {
		...
		for (int x = 0; x < numBuildings; x++) {
			...
			//create guiText object with class name
		
			//string classLabel;
			//guiText.text = javaClasses[0,x];
			//classLabel.transform.position = new Vector3(positions[x,0], heights[x]/2 + 0.75f, positions[x,1]);
			
			//Instantiate(classLabel, new Vector3(positions[x,0], heights[x]/2 + 0.75f, positions[x,1]), Quaternion.identity);
			//classLabel.guiText.text = javaClasses[0,x];
			...
			//clone = Instantiate (classLabel, new Vector3(positions[x,0], heights[x]/2 + 0.75f, positions[x,1]), Quaternion.identity);
			//clone.text = javaClasses[x,0];
			//clone.fontSize = 20;
		...
		Debug.Log ("KEVIN : arrayBuildingLabels.Count " + arrayBuildingLabels.Count); //CORRECT
	}
